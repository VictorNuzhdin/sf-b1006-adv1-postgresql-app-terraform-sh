...
CONFIGS_PATH=/home/ubuntu/scripts/configs/step04_nginx
LOG_PATH=$SCRIPTS_PATH/step04-nginx.log
NEW_USER_LOGIN=devops
WEBSITE_DOMAIN_NAME=vm2.dotspace.ru
...
echo "[$WEBSITE_DOMAIN_NAME] ($(curl -s http://169.254.169.254/latest/meta-data/public-ipv4))" >> $LOG_PATH
curl -s $(curl -s http://169.254.169.254/latest/meta-data/public-ipv4) | grep title | awk '{$1=$1;print}' >> $LOG_PATH
echo "" >> $LOG_PATH
